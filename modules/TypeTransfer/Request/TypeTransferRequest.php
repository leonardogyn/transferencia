...
namespace Modules\TypeTransfer\Request;

use Modules\TypeTransfer\Rule\ExcludeTypeTransferRule;
use Illuminate\Foundation\Http\FormRequest;

class TypeTransferRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        $rules_default = [
            'id' => [
                'required',
                'unique:type_transfers,id,NULL,id',
                'max:36'
            ],
            'name' => [
                'required',
                'unique:type_transfers,name,NULL,name',
                'max:100',
            ],
            'flag' => [
                'required',
                'unique:type_transfers,flag,NULL,flag',
                'max:1',
            ],
        ];
        ...
        elseif ($this->route()->getActionMethod() == 'update') {
            $rules_update = [
                'id' => [
                    'required',
                    'cnpj',
                    'unique:type_transfers,id,' . $this->id . ',id',
                    'string',
                    'max:36'
                ],
                'name' => [
                    'required',
                    'unique:type_transfers,name,' . $this->name . ',name',
                    'string',
                    'max:100',
                ],
                'flag' => [
                    'required',
                    'unique:type_transfers,flag,' . $this->flag . ',flag',
                    'string',
                    'max:100',
                ],
            ];
        ...
        elseif ($this->route()->getActionMethod() == 'delete') {
            // Regras de exclusão
            $rules_destroy = [
                'id' => new ExcludeTypeTransferRule(),
            ];
